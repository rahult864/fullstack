...
let headers=new HttpHeaders({
  Authorization :basicAuthHeaderString
    ...
    console.log("Payload = " + this.http.get<HelloWorldBean>(`http://localhost:8989/helloWorldBean/${name}`,{headers}).subscribe(
      response=>console.log(response)
    ));
    return this.http.get<HelloWorldBean>(`http://localhost:8989/helloWorldBean/${name}`,{headers});
